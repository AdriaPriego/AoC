...
int main() {
    ...
    while (search_start != corrupted_memory.cend()) {
        if(!std::regex_search(search_start, corrupted_memory.cend(), match, valid_mul_pattern) &&
        ...
        !std::regex_search(search_start, corrupted_memory.cend(), matchdont, dont_pattern))
            break;
        if (match.position(0) < matchdo.position(0) && match.position(0) < matchdont.position(0)) {
            if (mul_enabled) {
